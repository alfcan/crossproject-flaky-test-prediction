...
public class ReadabilityMetrics {
	...
	public double getMetric(MetricType t) throws IllegalArgumentException {
		...
		switch (t) {
		case BUSE_WEIMER:
			return getBuseWeimer();
		case BUSE_WEIMER_2:
			return getBuseWeimer2();
		case SMOG:
			return getSMOG();
		case FLESCH_READING:
	...
	public ReadabilityMetrics(String text) {
	...
	private double getBuseWeimer() {
		double readability = 0;
		readability = raykernel.apps.readability.eval.Main.getReadability(this.originalText);
		return readability;
	}
	
	private double getBuseWeimer2() {
		double readability = 0;
		readability = getComplex()/getSentences();
		return readability;
	}
